...
def memory_from_dict(data: dict[str, Any]) -> MemoryItem:
    item = MemoryItem(
        ...
        weight=data.get("weight") if data.get("weight") is not None else 5,
        ...
        access_count=data.get("access_count") if data.get("access_count") is not None else 0,
        expires_at=_dt_from_text(data.get("expires_at")),
        supersedes=data.get("supersedes") if data.get("supersedes") is not None else [],
        related=data.get("related") if data.get("related") is not None else [],
    )
