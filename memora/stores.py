...
class FileMemoryStore:
    ...
    def _item_from_text(self, text: str) -> MemoryItem:
        ...
        item = MemoryItem(
            ...
            confidence=meta.get("confidence") if meta.get("confidence") is not None else 1.0,
            weight=meta.get("weight") if meta.get("weight") is not None else 5,
            ...
            access_count=meta.get("access_count") if meta.get("access_count") is not None else 0,
            expires_at=_dt_from_text(meta.get("expires_at")),
            supersedes=meta.get("supersedes") if meta.get("supersedes") is not None else [],
            related=meta.get("related") if meta.get("related") is not None else [],
        )
